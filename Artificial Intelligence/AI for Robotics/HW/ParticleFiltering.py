...
class robot:
    ...
    def move(self, motion):  # Do not change the name of this function
        ...
        distance = motion[1]
        distance += random.gauss(0.0, res.distance_noise)
        #noisyDistance = random.gauss(distance, self.distance_noise)

        steering = motion[0]
        steering += random.gauss(0.0, res.steering_noise)
        #noisySteering = random.gauss(steering, self.steering_noise)

        beta = distance / res.length * tan(steering)

        if abs(beta) < 0.001:
            res.x += distance * cos(res.orientation)
            res.y += distance * sin(res.orientation)
            res.orientation = (res.orientation + beta) % (2 * pi)
        else:
            R = distance / beta
            cx = res.x - sin(res.orientation) * R
            cy = res.y + cos(res.orientation) * R
            res.x = cx + sin(res.orientation + beta) * R
            res.y = cy - cos(res.orientation + beta) * R
            res.orientation = (res.orientation + beta) % (2 * pi)
