...
class robot:
    ...
    def cte(self, radius):
        ...
        #
        if self.x < radius:
            distanceFromCenter = distance((self.x, self.y), (radius, radius))
            return distanceFromCenter - radius
        if self.x > 3 * radius:
            distanceFromCenter = distance((self.x, self.y), (3 * radius, radius))
            return distanceFromCenter - radius
        else:
            if self.y < 0:
                return - self.y
            elif self.y > 2 * radius:
                return self.y - 2 * radius
            else:
                return max(-self.y, self.y - 2 * radius)

############## ONLY ADD / MODIFY CODE ABOVE THIS LINE ####################

# Accepts two tuples or lists of length 2.
def distance(p1, p2):
