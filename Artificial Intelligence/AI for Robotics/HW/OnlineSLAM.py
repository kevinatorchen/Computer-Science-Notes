...
# Data Format:
#       list of senses and movements:
#           2-element-list of a single sense and movement:
#               list of landmarks sensed:
#                   3-element list of landmark:
#                       landmark-number
#                       x-distance
#                       y-distance
#               2-element list for movement:
#                   x-movement
#                   y-movement
...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    #
    Omega = matrix()
    matrix_dimension = 2 * (num_landmarks + 1)
    Omega.zero(matrix_dimension, matrix_dimension)

    xi = matrix()
    xi.zero(matrix_dimension, 1)
    ...
    for current_data in data:
        landmarks = current_data[0]
        for landmark in landmarks:
            landmark_number = landmark[0]
            x_distance = landmark[1]
            y_distance = landmark[2]
            landmark_index_x = 2 + 2 * landmark_number
            landmark_index_y = landmark_index_x + 1
            position_index_x = 0
            position_index_y = 1

            Omega.value[landmark_index_x][landmark_index_x] += 1 / measurement_noise
            Omega.value[landmark_index_x][position_index_x] -= 1 / measurement_noise
            xi.value[landmark_index_x][0] += x_distance / measurement_noise
            Omega.value[position_index_x][position_index_x] += 1 / measurement_noise
            Omega.value[position_index_x][landmark_index_x] -= 1 / measurement_noise
            xi.value[position_index_x][0] -= x_distance / measurement_noise

            Omega.value[landmark_index_y][landmark_index_y] += 1 / measurement_noise
            Omega.value[landmark_index_y][position_index_y] -= 1 / measurement_noise
            xi.value[landmark_index_y][0] += y_distance / measurement_noise
            Omega.value[position_index_y][position_index_y] += 1 / measurement_noise
            Omega.value[position_index_y][landmark_index_y] -= 1 / measurement_noise
            xi.value[position_index_y][0] -= y_distance / measurement_noise

        movement = current_data[1]
        x_movement = movement[0]
        y_movement = movement[1]

        expand_list = [0, 1]
        for i in range(4, matrix_dimension + 2):
            expand_list.append(i)
        Omega_expanded = Omega.expand(matrix_dimension + 2, matrix_dimension + 2, expand_list, expand_list)
        xi_expanded = xi.expand(matrix_dimension + 2, 1, expand_list, [0])

        init_position_index_x = 0
        init_position_index_y = 1
        final_position_index_x = 2
        final_position_index_y = 3

        Omega_expanded.value[final_position_index_x][final_position_index_x] += 1 / motion_noise
        Omega_expanded.value[final_position_index_x][init_position_index_x] -= 1 / motion_noise
        xi_expanded.value[final_position_index_x][0] += x_movement / motion_noise
        Omega_expanded.value[init_position_index_x][init_position_index_x] += 1 / motion_noise
        Omega_expanded.value[init_position_index_x][final_position_index_x] -= 1 / motion_noise
        xi_expanded.value[init_position_index_x][0] -= x_movement / motion_noise

        Omega_expanded.value[final_position_index_y][final_position_index_y] += 1 / motion_noise
        Omega_expanded.value[final_position_index_y][init_position_index_y] -= 1 / motion_noise
        xi_expanded.value[final_position_index_y][0] += y_movement / motion_noise
        Omega_expanded.value[init_position_index_y][init_position_index_y] += 1 / motion_noise
        Omega_expanded.value[init_position_index_y][final_position_index_y] -= 1 / motion_noise
        xi_expanded.value[init_position_index_y][0] -= y_movement / motion_noise

        B = Omega_expanded.take([0, 1], [0, 1])
        keep_list = []
        for i in range(2, matrix_dimension + 2):
            keep_list.append(i)
        A = Omega_expanded.take([0, 1], keep_list)
        C = xi_expanded.take([0, 1], [0])
        Omega_prime = Omega_expanded.take(keep_list, keep_list)
        xi_prime = xi_expanded.take(keep_list, [0])
